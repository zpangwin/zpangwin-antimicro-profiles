...
# create backup
cp -a "${TARGET_REG_FILE}" "${TARGET_REG_FILE}.$(date +'%Y-%m-%d_%H%M%S').before-disabling.bak"
...
elif [[ 1 == "${isSDLEnabled}" ]]; then
	sed -Ei 's/^("Enable SDL"=dword):.*$/\1:00000000/g' "${TARGET_REG_FILE}";
fi
...
if [[ 1 == "${hasRegKey}" && 1 == "${hasEnableSDL}" && 0 == "${isSDLEnabled}" ]]; then
	status='SUCCESS';
fi
...
echo "";
echo "${status}: SDL should be disabled, preventing Proton/Wine to pass gamepads through to game process.";
echo "";
